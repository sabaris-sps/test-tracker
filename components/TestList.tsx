...
import { Trash2, Calendar, Target, History, AlertCircle, Clock, Book, Glasses, Brain, Calculator } from 'lucide-react';
...
const TestList: React.FC<TestListProps> = ({ entries, onDelete }) => {
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
        ...
        <h3 className="text-lg font-semibold text-slate-800">No test records found</h3>
        <p className="text-slate-500 text-center max-w-xs mt-1">Start adding your test results to build up your performance history.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {entries.map((entry) => (
          <div key={entry.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h4 className="font-bold text-slate-900 text-lg">{entry.testName}</h4>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-sm text-slate-500">
                  <span className="flex items-center gap-1.5"><Calendar size={14} className="text-slate-400" /> {new Date(entry.date).toLocaleDateString()}</span>
                  <span className="flex items-center gap-1.5 font-semibold text-indigo-600">Total: {entry.total.marks}/300</span>
              ...
              <button
                ...
                className="text-slate-400 hover:text-red-600 p-2 rounded-xl hover:bg-red-50 transition-all self-end sm:self-center"
                title="Delete record"
              >
                <Trash2 size={20} />
              </button>
            ...
            <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-6">
              {['physics', 'chemistry', 'maths'].map((subj) => {
                ...
                return (
                  <div key={subj} className="bg-slate-50/30 p-4 rounded-2xl border border-slate-100 flex flex-col h-full">
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-xs font-black uppercase tracking-widest text-slate-400">{subj}</span>
                      <div className="flex items-baseline gap-1">
                        <span className="text-lg font-bold text-slate-900">{s.marks}</span>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-auto pt-3 border-t border-slate-100">
                      ...
                      {s.calcError === 0 && s.misconception === 0 && s.conceptNotAware === 0 && s.readingError === 0 && s.extraThinking === 0 && s.lackOfTime === 0 && (
                        <span className="text-[9px] text-slate-400 italic">No specific errors logged</span>
                      )}
                    </div>
                  </div>
                );
